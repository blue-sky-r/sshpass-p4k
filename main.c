...
static void show_help()
{
    printf("Usage: " PACKAGE_NAME " [-f|-d|-p|-e] [-hV] command parameters\n"
	    ...
	    "   -p pass       Provide password or passphrase as argument (security unwise)\n"
...
int handleoutput( int fd )
{
    ...
    static int state0, state1, state2;
    static const char compare0[]="passphrase for key";  // Asking for a passphrase
    ...
    if( compare0[state0]=='\0' ) {
        if( prevmatch ) {
            // Wrong passphrase - terminate with proper error code
            return RETURN_INCORRECT_PASSPHRASE;
        } else {
            write_pass( fd );
            state0=0;
            prevmatch=1;
